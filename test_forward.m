...
for i = 1:size(site_x, 2)
    [Hax(i,1), Za(i, 1), Delta_T(i,1)] = magnetic_froward_2D(site_x(i), site_z, x1, x2, z1, z2, M, Is);
    % Delta_T(i,1) = Hax(i,1) * cos(pi * I0 / 180) + Za(i, 1)*sin(pi * I0 / 180);
    % Delta_T(i,1) = sqrt(Hax(i,1)^2 + Za(i,1)^2);
end

for i = 1:size(site_x, 2)
    [Hax_Guan(i,1), Za_Guan(i, 1), Delta_T_Guan(i,1)] = magnetic_froward_2D_Guan(site_x(i), site_z, x1, x2, z1, z2, M, Is);
    % Delta_T(i,1) = Hax(i,1) * cos(pi * I0 / 180) + Za(i, 1)*sin(pi * I0 / 180);
    % Delta_T(i,1) = sqrt(Hax(i,1)^2 + Za(i,1)^2);
end
    [Hax_vector, Za_vector, Delta_T_vector] = magnetic_froward_2D(site_x, site_z, x1, x2, z1, z2, M, Is);
% for i = 1:size(site_x, 2)
